...
public class Calculator {
    ...
    public String infixToPostfix(String infix, IStack<Character> stack) {
        ...
        // Push a special character (using # as per instructions)
        stack.push('#');
        ...
        for (int i = 0; i < infix.length(); i++) {
            ...
            if (Character.isLetterOrDigit(ch)) {
                ...
                // If the number has multiple digits, we should group them, but assuming single
                // digit logic
                // as per typical homework, or handle multi-digit:
                // Let's handle multi digit by peeking next characters if they are digits.
                while (i + 1 < infix.length() && Character.isDigit(infix.charAt(i + 1))) {
                    postfix.deleteCharAt(postfix.length() - 1); // remove space
